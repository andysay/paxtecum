...
#if defined(NDEBUG)
# error "Paxtecum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xeb850cc6e37c501b67b66c196516b1da0daa39d30e22f9f46dac853533c1823f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Paxtecum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Paxtecum Signed Message:\n";
...
{
    // Paxtecum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Paxtecum: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Paxtecum: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Paxtecum: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Paxtecum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Paxtecum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5d61297aed532e384f96e444c6cd505ca888c3468c5b5cc87f843e389b961500");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PAXTECUM DEVELOPED BY ANDREY LEONOV IN YALTA CITY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1520417777;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088257069;

        if (fTestNet)
        {
            block.nTime    = 1520417776;
            block.nNonce   = 385710808;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1642c723aba458b740c72880ca1acb0342b40f799edf9511537a1bfb8a7e4779"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Paxtecum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PaxtecumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PaxtecumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PaxtecumMiner(CWallet *pwallet)
{
    printf("PaxtecumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paxtecum-miner");
    ...
    try { loop {
        ...
        printf("Running PaxtecumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PaxtecumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PaxtecumMiner, pwallet));
}
